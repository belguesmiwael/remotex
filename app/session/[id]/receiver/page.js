...
export default function ReceiverPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const join = () => socket.emit(EVENTS.JOIN_SESSION, sessionId)
      const onSessionReady = () => setConnectionStatus('connected')
      const onPeerDisconnected = () => setConnectionStatus('peer-left')
      const onSessionEnded = () => setConnectionStatus('ended')
      const onError = (data) => { setErrorMsg(data?.message || 'Could not join session'); setConnectionStatus('error') }
  ...
  if (errorMsg) return <StatusScreen icon="⚠️" title="Error" message={errorMsg} onHome={() => router.push('/')} />
  if (connectionStatus === 'joining') return <StatusScreen icon="🔗" title="Joining session..." message="Connecting to the controller..." />
  if (connectionStatus === 'ended' || connectionStatus === 'disconnected') return <StatusScreen icon="🔌" title="Session Ended" message="The controller disconnected." onHome={() => router.push('/')} />
  if (connectionStatus === 'peer-left') return <StatusScreen icon="👋" title="Controller Left" message="The controller disconnected from this session." onHome={() => router.push('/')} />
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden">
      ...
      <div
        ...
        style={{ left: cursorPos.x, top: cursorPos.y, opacity: connectionStatus === 'connected' ? 1 : 0 }}
      />
      ...
      >
        <div className="glass-card px-4 py-2 flex items-center gap-3 text-xs">
          ...
          <span className="text-text-secondary">
            RemoteX Receiver ·
            <span className="text-violet-400 font-mono ml-1">{sessionId?.slice(0, 8).toUpperCase()}</span>
          </span>
          <button onClick={() => router.push('/')} className="text-red-400/70 hover:text-red-400 transition-colors ml-2">
            ✕ Disconnect
          </button>
        </div>
      </div>
      ...
      {commandLog.length > 0 && showUI && (
        <div className="fixed bottom-4 right-4 flex flex-col gap-1 z-50">
          {commandLog.slice(-5).map((entry) => (
            <div key={entry.id} className="log-entry glass-card px-3 py-1.5 text-xs font-mono flex items-center gap-2">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-8">
      <div className="glass-card p-10 text-center max-w-sm animate-slide-up">
        ...
        <h2 className="font-display text-xl font-semibold text-text-primary mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>{title}</h2>
        <p className="text-sm text-text-secondary mb-6">{message}</p>
        {onHome && (
          <button onClick={onHome} className="btn-glow px-6 py-2.5 rounded-xl text-white text-sm font-medium">
            Back to Home
          </button>
        )}
      </div>
